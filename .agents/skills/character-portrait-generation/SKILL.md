...
11. If the Codex session log contains an `image_generation_call`, use `$codex-session-log` to copy its `id` and `revised_prompt` into the sidecar. Do not treat `revised_prompt` as an improved prompt unless it differs from the submitted prompt for that call.
...
If generation happened in a persisted Codex session, the session JSONL may include an `image_generation_call` item with an `id`, `status`, `revised_prompt`, and base64 `result`. Use `$codex-session-log` for line-numbered extraction. Use that session item for the sidecar when available; do not paste the base64 result into the sidecar. Observed local behavior on 2026-04-25: `revised_prompt` may be byte-for-byte identical to the submitted prompt, and `status` may still say `"generating"` after `result` is present.
